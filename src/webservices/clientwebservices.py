...
class ClientWebServices:
    def __init__(self):
        ...
        self.logger = com_logger.Logger('Client WS')
    ...
    def inserttemphum(self, name, temperature, humidity):
        try:
            client = JSONWSPClient(self.url + '/SQLiteService/jsonwsp/description')
            jsonwsp_resp = client.inserttemphum(name = name, temp = temperature, hum = humidity)
            self.logger.debug('WebServices-' + client.path)
        except Exception as exp:
            self.logger.error(repr(exp))
